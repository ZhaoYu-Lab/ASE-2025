...
class Model(nn.Module):   
    def __init__(self, encoder,encoder_2,config,tokenizer,tokenizer_2,args):
        ...
        self.encoder_2 = encoder_2  #plbart
        self.config=config
    ...
    def forward(self, input_ids=None,input_ids_2=None,labels=None): 
        ...
        model_outputs_2 = self.encoder_2(input_ids= input_ids_2, attention_mask=attention_mask_2, labels=input_ids_2, decoder_attention_mask=attention_mask_2, output_hidden_states=True)
        hidden_states_2 = model_outputs_2['decoder_hidden_states'][-1]

        #######bos+eos plbart ########

        #bos => eos 互换分别得到第一个和最后一个 这一行代码创建了一个名为 eos_mask 的布尔型张量，用于指示哪些位置包含了T5的结束标记（<eos>）。这是通过检查输入 source_ids 是否等于T5配置中的结束标记ID来实现的。
        eos_mask_2 = input_ids_2.eq(self.encoder_2.config.eos_token_id)
        #print(np.shape(eos_mask))
        #print(eos_mask)
        if len(torch.unique(eos_mask_2.sum(1))) > 1:
           raise ValueError("All examples must have the same number of <eos> tokens.")

        #这一行代码根据 eos_mask 从隐藏状态中选择包含结束标记的位置，然后将这些位置的隐藏状态合并成一个向量。最后的结果是一个二维张量，每一行对应一个输入示例，每一行中的向量是该示例的编码表示。
        outputs_2 = hidden_states_2[eos_mask_2, :].view(hidden_states_2.size(0), -1, hidden_states_2.size(-1))[:, -1, :]
        #print(np.shape(outputs))
        #print(outputs)
        #######bos+eos plbart ########   
        ...
        logits_2 = self.classifier_2(outputs_2)
